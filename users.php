...
if( $_SERVER['REQUEST_METHOD'] == "GET"){
    ...
    if(isset($_GET['token'])){   
        ...
        $authtoken = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $dectoken);
        
        $authtoken = trim($authtoken);       


        $selectquer = $mysqli->query("SELECT * FROM user WHERE token='".$authtoken."'");
    ...
    //invoking python script to decrypt the AES 256 encrypted password sent from the client.

    $password = rawurldecode($password);
    
    $decpassword = exec('python aesdecrypt.py '.$password, $output);    

    //removing the characters that break the decrypted string and thus SQL Query.
    
    $pass = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $decpassword);

    $pass = trim($pass);   

    #$pass = iconv("ISO-8859-1", UTF-8, $decpassword);

    $selectquer = $mysqli->query("SELECT * FROM user WHERE username='".$username." 'and password='".$pass." ' ");
        ...
        while($row = $selectquer->fetch_assoc()) {
             ...
             $enctoken = shell_exec('python aesencrypt.py '.$token);
             
             $encryptedtoken = urlencode($enctoken);
            
             //$token = preg_replace('/[\x00-\x1f]/', '', $token);
             
             $file = fopen('output.txt', 'a');
             
             fwrite($file, $encryptedtoken);
...
//REGISTER CODE
    ...
    //url decoding is needed to properly decrypt the cipher text.

    $password = rawurldecode($password);

    $cnfpassword = rawurldecode($cnfpassword);
    ...
    $decryptedpassword = preg_replace('/[\x00-\x1f]/', '', $decpassword);

    $deccnfpassword = exec('python aesdecrypt.py '.$cnfpassword, $output);    

    //removing the characters that break the decrypted string and thus SQL Query.

    $decryptedcnfpassword = preg_replace('/[\x00-\x1f]/', '', $deccnfpassword);



    if($decryptedpassword!=$decryptedcnfpassword){   
    ...
    $insertquer = $mysqli->query("INSERT INTO `securestore`.`user` (`email`, `username`, `password`, `token`) 
         VALUES ('$email', '$username', '$decryptedpassword','$token')");
